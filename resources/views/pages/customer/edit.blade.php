...
@section('content')
<!--creat customer form -->
<form method="POST" class="center" action="/dashboard/customers/{{$customer->id}}">
  @method('patch')
  @csrf
  <!-- Customer info -->
  <h2 class="font-weight-bold text-primary">{{ __('Edit Customer :') }}</h2>
  <!-- Customer account -->
  <div class="form-row">
    <!-- account name -->
    <div class="form-group col-md-6">
      ...
      <input type="text" class="form-control {{ $errors->has('accountName') ? ' is-invalid' : '' }}"
        placeholder="Enter Account Name" name="accountName" id="accountName" value="{{$customer->accountName}}">
